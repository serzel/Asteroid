...
export class UIRenderer {
  ...
  drawTitleScreen(ctx, uiModel) {
    ...
    drawCircularGlow(ctx, uiModel.world.w * 0.5, uiModel.world.h * 0.79, 95, "rgba(112,233,255,0.55)", 0.26);
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
  ...
  #drawNeonTitle(ctx, text, x, y) {
    ...
    drawCircularGlow(ctx, x, y, 64, "rgba(255,84,238,0.95)", 0.4);
    ctx.fillStyle = grad;
  ...
  #drawNeonButton(ctx, rect, label, state) {
    ...
    drawOutlineGlow(ctx, (c) => this.#roundedRectPath(c, x, y, w, h, radius), "rgba(244,103,255,0.95)", 1.6, 0.32 + glowBoost * 0.3 + pulse * 0.12 + pressBoost * 0.28);
    ctx.lineWidth = 2.2;
    ctx.strokeStyle = borderGrad;
    this.#roundedRectPath(ctx, x, y, w, h, radius);
    ctx.stroke();
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    drawCircularGlow(ctx, x + w * 0.5, y + h * 0.5 + 1, Math.max(w, h) * 0.19, "rgba(127,245,255,0.85)", 0.24 + glowBoost * 0.18 + pressBoost * 0.2);
